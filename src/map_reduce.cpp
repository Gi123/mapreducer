...
void MapThread(const std::string& file_name, Block block, mapper& func, size_t prefix_count, MrVector& output) {
	std::ifstream ifs(file_name);

	ifs.seekg(block.from);

	std::string line;

	while (ifs.tellg() < std::streamoff(block.to)) {
		ifs >> line;
...
bool ReduceThread(MrVector& input, reducer& func) { return func(*input); }
...
bool MapReduce(const std::string& file_name, const std::vector<Block> ranges,
	size_t prefix_count, int rnum, mapper map_func, reducer reduce_func) {
	...
	for (const auto& pair : ranges)
		threads.push_back(std::thread(MapThread, ref(file_name), pair, std::ref(map_func),
